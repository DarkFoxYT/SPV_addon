...
public class IKEAWalkerRenderer extends DynamicGeoEntityRenderer<IkeaWalkerEntity> {

    public IKEAWalkerRenderer(EntityRendererFactory.Context context) {
        super(context, new IKEAWalkerModel());
    }

    @Override
    public void render(IkeaWalkerEntity entity, float entityYaw, float partialTick, MatrixStack poseStack, VertexConsumerProvider bufferSource, int packedLight) {
    ...
    @Override
    public void renderRecursively(MatrixStack poseStack, IkeaWalkerEntity animatable, GeoBone bone, RenderLayer renderType, VertexConsumerProvider bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
        if (bone == null) return;
        poseStack.push();
