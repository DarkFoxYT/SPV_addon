...
import net.dark.spv_addon.entities.ai.goals.AggroPlayer;
import net.dark.spv_addon.entities.ai.goals.LevitationGoal;
...
public class Sani_ty extends PathAwareEntity
        implements IKAnimatable<Sani_ty>, GeoAnimatable {

    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache((GeoAnimatable) this);
    private final IKLegCompDark<TargetReachingIKChain, Sani_ty> legComponent;

    public Sani_ty(EntityType<? extends Sani_ty> type, World world) {
        super(type, world);
        this.navigation = new SlightlyBetterMobNavigation(this, world);

        List<ServerLimb> endpoints = List.of(
                new ServerLimb( 1.5, 2.0,  2),
                new ServerLimb(-1.5, 2.0,  2),
                new ServerLimb( 1.8, 2.0,  0),
                new ServerLimb(-1.8, 2.0,  0),
                new ServerLimb( 1.5, 2.0, -2),
                new ServerLimb(-1.5, 2.0, -2)
        );
        ...
        TargetReachingIKChain chain = new TargetReachingIKChain(
                new Segment.Builder().length(0.70).build(),
                new Segment.Builder().length(1.00).build(),
                new Segment.Builder().length(1.30).build(),
                new Segment.Builder().length(0.90).build()
        );
    ...
    protected void initGoals() {
        // 0 = float up
        this.goalSelector.add(0, new LevitationGoal(this));
        ...
        // 3 = wander
        this.goalSelector.add(3, new SmartWanderGoal(this, 0.5, 20, 60));
        // 4 = agro nearest speaking player
        this.goalSelector.add(4, new AggroPlayer(this, 20.0));
    }
    ...
    @Override
    public List<IKModelComponent<Sani_ty>> getComponents() {
