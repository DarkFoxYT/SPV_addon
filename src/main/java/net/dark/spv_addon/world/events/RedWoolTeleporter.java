...
public class RedWoolTeleporter {
    ...
    public static final RegistryKey<World> SOURCE_WORLD = BackroomsLevels.LEVELRUN_WORLD_KEY;

    public static void tickPlayer(ServerPlayerEntity player) {
        // Only run this in the correct world
        if (!player.getWorld().getRegistryKey().equals(SOURCE_WORLD)) return;

        BlockPos playerPos = player.getBlockPos();

        // Scan 11x11x11 cube around the player
    ...
    public static void teleportPlayer(ServerPlayerEntity player) {
        ...
        // Basic teleport with yaw/pitch preserved
